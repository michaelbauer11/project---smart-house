...
def updatingDeviceStatusInJsonFile(houseToDevices, current_status, device):
    ...
    houseToDevices[device]["status"] = current_status
    with open('data.json') as infile:
       file = json.load(infile)
       file = ast.literal_eval(file)
       file[device]["status"] = current_status
    with open('data.json', 'w') as outfile:
                ...
                for key, field in fields.iteritems():
                    ...
                    if key == 'temp_activate_outside' and field != 'none':
                        ...
                        if int(statsManager.get_temperature()) > int(field):
                            print "yeahhhhh temp"
                            self.houseToDevices[device]['communication'].send('1')
                            updatingDeviceStatusInJsonFile(self.houseToDevices,"1", device)
                            print int(field)
                        ...
                        for time_lim in time_limits:
                            ...
                            if filter(str.isdigit, time_lim) != '':
                                ...
                                print int(statsManager.get_current_time())
