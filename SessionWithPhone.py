        ...
        try:
            ...
            while not identity_verified:
                ...
                if input[0] == "signin":
                    for user_name, user_passward in users_table.iteritems():
                        if str(user_name).rstrip() == str(input[1]).rstrip() and str(user_passward).rstrip() == str(input[2]).rstrip():
                            identity_verified = True
                            self.phone_sock.send("confirmed"+"\n")
                    ...
                    if input[3] == "secretkey":
                        ...
                        print "vconfirmed"
                        identity_verified = True
                        users_table[str(input[1]).rstrip()] = str(input[2]).rstrip()
            ...
            while True:
                ...
                if data[0].startswith('device'):
                    ...
                    print data[0]
                    print data[1]
                    self.phoneManager.deviicesManager.houseToDevicesTable.houseToDevices[data[0]]['communication'].send(data[1])
                    print 'sent'
                    #update dictionary:
